---
title: "`r display_name` <img src=\"Logo.png\" height=\"80px\" style=\"float: right;\"/>"
...
tech_raw <- read_sheet("tech_sheet_url") %>% select(-"Txt.Sum", -"Email", -"Name", -"Date") %>% 
  mutate(Date = as.Date(What.is.the.date.of.the.treatment., "%m/%d/%y"),
         Pt.ID = tolower(Pt.ID),
         Tx = as.integer(Tx),
         X = as.integer(X)) %>% 
  rename(Planned_Course = X) %>% 
  filter(Column1 != "xx")
survey_raw <- read_sheet("survey_sheet_url") %>% select(-"Txt.Sum", -"Email", -"Name", -contains(".of.16."),
...
tech_filtered <- tech_raw %>% filter(Pt.ID %in% selected_patient)
survey_filtered <- survey_raw %>% filter(Patient %in% selected_patient)
...
if (rounds == TRUE) {
  demographics <- read_sheet("demographics_sheet_url") %>% 
    mutate(Patient.ID = tolower(Patient.ID)) %>% 
    rename(Diagnoses = Diagnoses..write.out.in.full.) %>% 
    filter(Patient.ID %in% selected_patient)
  sprintf("The patient (ID = '%s') is a %i-year-old %s with %s. The patient has completed %i of %i planned sessions.", demographics$Patient.ID, demographics$Age, demographics$Gender, demographics$Diagnoses, tail(tech_filtered, 1)$Tx, tail(tech_filtered, 1)$Planned_Course)
}
...
tech_counts <- tech_filtered %>% select(Date, Pt.ID, Tx, Planned_Course, Protocol) %>% 
  group_by(Date) %>% 
...
phq9gad7 <- survey_filtered %>% filter(SurveyID == "PHQ9" | SurveyID == "GAD7") %>%
  group_by(Date, Patient) %>%
  summarise(PHQ9 = PHQ9[which(!is.na(PHQ9))[1]],
            PHQ9_Q9 = PHQ.Q9_[which(!is.na(PHQ.Q9_))[1]],
...
if (nrow(phq9gad7) != 0) {
  ...
  master_graph_data <- merge(tech_counts, master_graph_data, by = "Date", all = TRUE, no.dups = FALSE) %>% 
    select(-Patient, -Tx, -num_tx_on_day)
  ...
  master_graph_plot <- ggplot(master_graph_data, aes(x = Date, y = cumulative_num_tx)) +
    ...
    scale_color_manual(values = c("#97afc9", "#d6e5f0", "#b29056", "#77928b"), na.translate = FALSE) +
    scale_fill_manual(values = c("#97afc9", "#d6e5f0", "#b29056", "#77928b"), na.translate = FALSE) +
    new_scale_color() +
...
```{r, fig.width = 6, fig.height = 4, fig.show = "hold", out.width = "50%", results = "asis"}
## Filter BDI data
bdi <- survey_filtered %>% filter(SurveyID == "BDI")
last_date <- max(bdi$Completion.time)
bdi <- bdi %>% mutate(Completion.time = as.factor(as.POSIXct(Completion.time, format = "%m/%d/%y %I:%M %p", tz = "UTC"))) %>% 
  select(Completion.time, Patient, BDI_)

## If patient has BDI data, produce plot
if (nrow(bdi) != 0) {
  ## Prepare x-axis labels
  bdi_dates <- as.vector(sub(" .*", "", bdi$Completion.time))
  ...
  for (i in 1:length(indices_of_unique_bdi_dates)) {
    bdi_tick_marks <- append(bdi_tick_marks, as.vector(bdi$Completion.time)[i])
    bdi_tick_marks_labels <- append(bdi_tick_marks_labels, bdi_dates[i])
  }
  
  ## Plot BDI data
  bdi_plot <- ggplot(bdi, aes(x = Completion.time, y = BDI_)) +
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 29, ymax = 63, fill = "red", color = NA, alpha = 0.3) +
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 20, ymax = 29, fill = "orange", color = NA, alpha = 0.3) +
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 14, ymax = 20, fill = "yellow", color = NA, alpha = 0.3) +
    annotate("text", x = last_date, y = 46, label = "SEVERE", color = "#444444", size = 2) +
    annotate("text", x = last_date, y = 24.5, label = "MODERATE", color = "#444444", size = 2) +
    annotate("text", x = last_date, y = 17, label = "MILD", color = "#444444", size = 2) +
    ...
    theme_classic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_text(data = slice(bdi, n()), aes(label = BDI_), position = position_dodge(width = 0.9), vjust = 2, colour = "white")
...
madrs_raw <- read_sheet("MADRS_sheet_url")
...
## Filter QIDS data
qids <- survey_filtered %>% filter(SurveyID == "QIDS")
last_date <- max(qids$Completion.time)
qids <- qids %>% mutate(Completion.time = as.factor(as.POSIXct(Completion.time, format = "%m/%d/%y %I:%M %p", tz = "UTC"))) %>% 
  select(Completion.time, Patient, QIDS.Reporting)

## If patient has QIDS data, produce plot
if (nrow(qids) != 0) {
  ## Prepare x-axis labels
  qids_dates <- as.vector(sub(" .*", "", qids$Completion.time))
  ...
  for (i in 1:length(indices_of_unique_qids_dates)) {
    qids_tick_marks <- append(qids_tick_marks, as.vector(qids$Completion.time)[i])
    qids_tick_marks_labels <- append(qids_tick_marks_labels, qids_dates[i])
  }
  
  ## Plot QIDS data
  qids_plot <- ggplot(qids, aes(x = Completion.time, y = QIDS.Reporting)) +
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 16, ymax = 27, fill = "red", color = NA, alpha = 0.3) +
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 11, ymax = 16, fill = "orange", color = NA, alpha = 0.3) +
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 6, ymax = 11, fill = "yellow", color = NA, alpha = 0.3) +
    annotate("text", x = last_date, y = 21.5, label = "SEVERE", color = "#444444", size = 2) +
    annotate("text", x = last_date, y = 13.5, label = "MODERATE", color = "#444444", size = 2) +
    annotate("text", x = last_date, y = 8.5, label = "MILD", color = "#444444", size = 2) +
    geom_bar(stat = "identity", fill = "#364c63", colour = "#364c63") +
    coord_cartesian(ylim = c(0, 27)) +
    scale_x_discrete("Date and Time", breaks = qids_tick_marks, labels = qids_tick_marks_labels) +
    ylab("Score") +
    ggtitle("Quick Inventory of Depression Symptomatology (QIDS)") +
    theme_classic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_text(data = slice(qids, n()), aes(label = QIDS.Reporting), position = position_dodge(width = 0.9), vjust = 2, colour = "white")
...
dailies_chart <- survey_filtered %>% filter(SurveyID == "Pre-TMS Survey")

if (nrow(dailies_chart) != 0) {
  ## Convert responses to integers
  dailies_chart <- dailies_chart %>% rename(Depression = Over.the.last.day..have.you.felt.down..depressed...sad...1.of.7.,
  ...
  weekly_chart <- dailies_chart %>% mutate(Date = as.Date(`Date and Time`, "%m/%d/%y"),
...
```{r}
## Filter hourly data
hourlies_chart <- survey_filtered %>% filter(SurveyID == "Hourly Mood Survey")

if (nrow(hourlies_chart) != 0) {
  ## Convert responses to integers
  hourlies_chart <- hourlies_chart %>% rename(Depression = Over.the.last.hour..have.you.felt.down...depressed..sad.,
  ...
  daily_average_chart <- hourlies_chart %>% mutate(Date = as.Date(`Date and Time`, "%m/%d/%y")) %>% 
    rowwise() %>% 
    mutate(positive_emotions = mean(c(Happiness, Peacefulness, Hopefulness))) %>% 
    group_by(Date) %>% 
    summarise(`Avg. Depression` = round(mean(Depression, na.rm = TRUE), 1),
...
## Convert responses to integers
pre_tms_other <- survey_filtered %>% filter(SurveyID == "Pre-TMS Survey") %>% 
  ...
  mutate(sleep_hours = as.integer(sleep_hours),
         ...
         Date = as.factor(Date)) %>% 
  select(Start.time, Date, Patient, sleep_hours, sleep_quality, pre_tms_suicide)
