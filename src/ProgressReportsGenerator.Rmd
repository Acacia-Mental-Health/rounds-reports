...
## gs4_deauth(clear_cache = TRUE, verbose = TRUE)
...
if (rounds == TRUE) {
  demographics <- read_sheet("***REMOVED***", sheet="Patient Demographics") %>% 
    ...
    filter(pt_id %in% tolower(selected_patient))
  sprintf("The patient (ID = '%s') is a %i-year-old %s with %s. The patient has completed %i of %s planned sessions.", demographics$pt_id, demographics$Age, demographics$Gender, demographics$Diagnoses, tail(tech_filtered, 1)$Tx, as.character(tail(tech_filtered, 1)$Planned_Course))
}
...
if (nrow(bdi) != 0) {
  ...
  bdi_plot <- ggplot(bdi, aes(x = Timestamp, y = BDI)) +
    ...
    annotate("text", x = as.POSIXct(Inf), y = 46, label = "SEVERE", color = "#444444", size = 2) +
    annotate("text", x = as.POSIXct(Inf), y = 24.5, label = "MODERATE", color = "#444444", size = 2) +
    annotate("text", x = as.POSIXct(Inf), y = 17, label = "MILD", color = "#444444", size = 2) +
    ...
    scale_x_datetime(breaks = as.POSIXct(bdi_tick_marks))
...
MADRS_SR <- survey_filtered %>% filter(!is.na(MADRS))
last_date <- max(bdi$Timestamp)
# bdi <- bdi %>% mutate(Timestamp = as.factor(as.POSIXct(Timestamp, format = "%m/%d/%y %I:%M %p", tz = "UTC"))) %>%
MADRS_SR <- MADRS_SR %>% select(Timestamp, pt_id, MADRS)
