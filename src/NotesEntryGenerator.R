...
%d total sessions each separated by at least 45 minutes.<br />
<br />"
  tms_end_template <- "<br />
Course:  Session # %s of %s planned.<br />
/////-----Patient Mood Surveys-----/////<br />
\"Date: %s -"
  ...
  if(nrow(tms_sessions_day) == 1) {
    ...
    tms_end <- sprintf(tms_end_template, 
                       if (is.integer(tms_sessions_day[1,][c('Treatment #')][[1]])) as.character(tms_sessions_day[1,][c('Treatment #')][[1]]) else "x",
                       if (is.integer(tms_sessions_day[1,][c('Planned Course')][[1]])) as.character(tail(tms_sessions_day[c('Treatment #')][[1]], 1)) else "x",
                       cur_date)
  } else {
    ...
    tms_end <- sprintf(tms_end_template,
                       ...
                       if (!is.na(tms_sessions_day[1,][c('Planned Course')][[1]])) as.character(tail(tms_sessions_day[c('Treatment #')][[1]], 1)) else "x",
                       cur_date)
  }
...
generate_surveys <- function(cur_date, survey_results_filtered) {
  ...
  for (r in 1:nrow(survey_results_filtered)) {
    c <- 3
    while (c <= 9) {
      if (!is.na(survey_results_filtered[r, c][[1]])) {
        output <- paste0(output, sprintf("Date: %s - %s = %d\n<br />", 
                                         cur_date, colnames(survey_results_filtered)[c],
...
survey_raw_tms_old <- read_sheet("***REMOVED***", sheet="Form Responses 1") %>%
...
survey_raw_tms_current <- read_sheet("***REMOVED***", sheet="Query") %>%
  filter(!is.na(`Over the last day, have you felt down/ depressed / sad? [1 of 7]`)) %>%
  rename(pt_id = 'ID') %>%
...
survey_raw_results_old <- read_sheet("***REMOVED***", sheet="Results") %>%
  select(c('Timestamp',
           'Patient Code',
           'QIDS',
           'BDI',
...
survey_raw_results_current <- read_sheet("***REMOVED***", sheet="Main Sheet") %>%
  select(c('Timestamp',
           'Patient Code',
           'QIDS',
           'BDI',
...
args <- c("7/1/22", "7/12/22", "IsBo18", "GrNo94")
for (patient in tail(args, -2)) {
  tech_filtered <- tech_raw %>% filter(tolower(pt_id) == tolower(patient),
                                       Date >= as.Date(args[1], "%m/%d/%y"),
                                       as.Date(args[2], "%m/%d/%y") >= Date)
  survey_results_filtered <- survey_raw_results %>% filter(tolower(pt_id) == tolower(patient),
                                       Timestamp >= as.Date(args[1], "%m/%d/%y"),
                                       as.Date(args[2], "%m/%d/%y") >= Timestamp)
  survey_tms_filtered <- survey_raw_tms %>% filter(tolower(pt_id) == tolower(patient),
                                       Date >= as.Date(args[1], "%m/%d/%y"),
                                       as.Date(args[2], "%m/%d/%y") >= Date)
